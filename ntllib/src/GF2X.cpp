...
/** Arnon's addition to count field operations: */
#ifdef NTL_OPS_COUNTER
//#define COUNT_ARITH 1   //Arnon

long long NTL::OpsCounter::additionsCounter = 0;
long long NTL::OpsCounter::multiplicationsCounter = 0;
long long NTL::OpsCounter::divisionsCounter = 0;
bool NTL::OpsCounter::count = false;
#endif
...
{
   if (i < 0) return to_GF2(0);						//Illegal index 
   long wi = i/NTL_BITS_PER_LONG;					//wi - The relevant word index.
   if (wi >= a.xrep.length()) return to_GF2(0);		//Illegal word
   long bi = i - wi*NTL_BITS_PER_LONG;				//bi  -The relevant bit inside the relevant word.

   //a.xrep[wi] - The relevant word in the word vector.
   //1UL << bi = 2^bi.
...
//#ifdef USE_STATIC_WV
//   int relevantWord = n-1;
//   while (a==0) {
//	   if (relevantWord==0)		///Degree is 0.
//		   return 0;
//	   relevantWord--;
//	   a = aa.xrep[relevantWord];
//   }
//#else
...
//#ifdef USE_STATIC_WV
//   return NTL_BITS_PER_LONG*(relevantWord-1) + i - 1;
//#else
...
#ifdef USE_STATIC_WV
   static WordVectorStatic mem;
   static WordVectorStatic stk;
   static WordVectorStatic vec;
#else
   WordVector mem;
   WordVector stk;
   WordVector vec;
