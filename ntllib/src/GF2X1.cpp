...
//#include "../PCPCD/src/Refactoring/ntlOMP.hpp"
#include <NTL/GF2X.h>
#include <NTL/vec_long.h>

#include <NTL/new.h>
#include <iostream> //Arnon
using namespace std;//Arnon
...
/********** variables needed for OpenMP multithreading   ************/

extern "C" const unsigned int ntlMaxNumThreads = 128; /* Max number of threads to be used in parallel.
                                                         This is currently used by the PCPCD FFT.
                                                         Defined as extern "C" to avoid C++ name mangling. 
                                                       */
...
extern "C" __declspec(thread) unsigned int ntlGlobalThreadNum = 0; // Defined as extern "C" to avoid C++ name mangling.
#else
extern "C" __thread unsigned int ntlGlobalThreadNum = 0; // Defined as extern "C" to avoid C++ name mangling.
//#pragma omp threadprivate(ntlGlobalThreadNum)
#endif

/********** data structures for accesss to GF2XRegisters ************/

static GF2X GF2XRegisterVec[ntlMaxNumThreads][32]; //Changed to enable multi-threading --shaul
static long GF2XRegisterTop[ntlMaxNumThreads] = {0};


class GF2XRegisterType { //Changed to enable multi-threading. --shaul
public:

GF2X *xrep;

GF2XRegisterType()
{ xrep = &GF2XRegisterVec[ntlGlobalThreadNum][GF2XRegisterTop[ntlGlobalThreadNum]]; GF2XRegisterTop[ntlGlobalThreadNum]++; }

~GF2XRegisterType()
{ xrep->xrep.release();  
  GF2XRegisterTop[ntlGlobalThreadNum]--; }

operator GF2X& () { return *xrep; }

};

#define GF2XRegister(a) GF2XRegisterType GF2XReg__ ## a ; GF2X& a = GF2XReg__ ## a	//Serial
//#define GF2XRegister(a) GF2X(a)													//Temp workaround. Highly degrades run time.



static vec_GF2X stab;  // used by PlainDivRem and PlainRem

#ifdef USE_STATIC_WV // TODO (shaul): static WV was never finished. Created by Arnon, was supposed to speed run time by allowing less allocations.
//	static WordVectorStatic GF2X_rembuf; //Changed to local variable for thread-safety //shaul
//	#pragma omp threadprivate (GF2X_rembuf)
	#define WORDVECTYPE WordVectorStatic
#else
//	static WordVector GF2X_rembuf; // TODO ( shaul ): Changed to local variable for thread-safety. Can now be changed to an array of variables (for each thread) for optimization. 
//	#pragma omp threadprivate (GF2X_rembuf)

// Currently GF2X_rembuf is defined as a local WordVector variable whenever needed. This seems not to impact performance. 
// This can be optimized in the future by:
// Change GF2X_rembuf to an array GF2X_rembuf[ntlMaxNumThreads] and change all occurences of GF2X_rembuf to GF2X_rembuf[ntlGlobalThreadNum] in order for each thread to hold a copy.

    #define WORDVECTYPE WordVector
#endif
...
void PlainDivRem(GF2X& q, GF2X& r, const GF2X& a, const GF2X& b)
{
   ...
   for (i = 0; i <= min(dq, NTL_BITS_PER_LONG-1); i++) {
	...
	#ifdef USE_STATIC_WV
		   WordVectorStatic& st = stab[((_ntl_ulong)(posb+i))%NTL_BITS_PER_LONG].xrep;
	#else
   ...
   stab_cnt(0), stab_ptr(0), stab1(0), tracevec(F.tracevec)
{
   if (method == GF2X_MOD_SPECIAL) {
      ...
      if (F.stab_ptr) {
         ...
         for (i = 0; i < NTL_BITS_PER_LONG; i++) {
			#ifdef USE_STATIC_WV   
						 WordVectorStatic& st = stab[((_ntl_ulong)(posn+i))%NTL_BITS_PER_LONG].xrep;
			#else
...
void InnerProduct(GF2X& x, const GF2X& v, long dv, long low, long high, 
                   const vec_GF2X& H, long n, WordVectorStatic& t)
{
   long i, j;
   ...
   for (;;) {
      if (vv & msk) {
		#ifdef USE_STATIC_WV
					const WordVectorStatic& h = H[i-low].xrep;
		#else
...
#ifdef USE_STATIC_WV
   WordVectorStatic scratch(INIT_SIZE, F.size);
#else
