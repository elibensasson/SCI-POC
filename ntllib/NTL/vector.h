...
typedef long indexType;		///Added by the PCPCD project (used to be long)
...
void BlockConstruct(T* l__p, indexType l__n)  \
{  \
   indexType l__i;  \
  \
  ...
  \
void BlockDestroy(T* l__p, indexType l__n)  \
{  \
   indexType l__i;  \
  \
   for (l__i = 0; l__i < l__n; l__i++)  \
...
public:  \
   T *_vec__rep;  \
  \
   void RangeError(indexType l__i) const;  \
  \
   vec_T() { _vec__rep = 0; }  \
   vec_T(NTL_NNS INIT_SIZE_TYPE, indexType l__n) { _vec__rep = 0; SetLength(l__n); }  \
   vec_T(const vec_T& l__a) { _vec__rep = 0; *this = l__a; }     \
   vec_T& operator=(const vec_T& l__a);  \
   ~vec_T();  \
   void kill(); \
  \
   void SetLength(indexType l__n);  \
   void SetMaxLength(indexType l__n); \
   void FixLength(indexType l__n); \
   void QuickSetLength(indexType l__n) { NTL_VEC_HEAD(_vec__rep)->length = l__n; } \
  \
   indexType length() const { return (!_vec__rep) ?  0 : NTL_VEC_HEAD(_vec__rep)->length; }  \
   indexType MaxLength() const { return (!_vec__rep) ?  0 : NTL_VEC_HEAD(_vec__rep)->init; } \
   indexType allocated() const { return (!_vec__rep) ?  0 : NTL_VEC_HEAD(_vec__rep)->alloc; } \
   indexType fixed() const { return _vec__rep && NTL_VEC_HEAD(_vec__rep)->fixed; } \
  \
   T& operator[](indexType l__i)   \
   {  \
      NTL_RANGE_CHECK_CODE  \
      return _vec__rep[l__i];  \
   }  \
  \
   const T& operator[](indexType l__i) const \
   {  \
      NTL_RANGE_CHECK_CODE  \
      return _vec__rep[l__i];  \
   }  \
  \
   T& RawGet(indexType l__i)   \
   {  \
      return _vec__rep[l__i];  \
   }  \
  \
   const T& RawGet(indexType l__i) const \
   {  \
      return _vec__rep[l__i];  \
   }  \
  \
   T& operator()(indexType l__i) { return (*this)[l__i-1]; }  \
   const T& operator()(indexType l__i) const { return (*this)[l__i-1]; } \
   \
  \
 ...
 \
   ...
   indexType position(const T& l__a) const;  \
   indexType position1(const T& l__a) const;  \
};  \
 \
...
indexType operator==(const vec_T& l__a, const vec_T& l__b);  \
indexType operator!=(const vec_T& l__a, const vec_T& l__b);
...
indexType vec_T::position(const T& l__a) const  \
{  \
   if (!_vec__rep) return -1;  \
   indexType l__num_alloc = NTL_VEC_HEAD(_vec__rep)->alloc;  \
   indexType l__num_init = NTL_VEC_HEAD(_vec__rep)->init;  \
   if (&l__a < _vec__rep || &l__a >= _vec__rep + l__num_alloc) return -1;  \
   indexType l__res = (&l__a) - _vec__rep;  \
   \
  ...
  \
indexType vec_T::position1(const T& l__a) const  \
{  \
   if (!_vec__rep) return -1;  \
   indexType l__len = NTL_VEC_HEAD(_vec__rep)->length;  \
...
#define NTL_vector_impl_plain(T,vec_T)  \
 \
void vec_T::SetLength(indexType l__n)   \
{   \
   indexType l__m;  \
  \
   if (l__n < 0) {  \
  ...
  \
      ...
      if (l__n > NTL_VEC_HEAD(_vec__rep)->alloc) {  \
         l__m = NTL_NNS max(l__n, indexType(NTL_VectorExpansionRatio*NTL_VEC_HEAD(_vec__rep)->alloc));  \
 ...
 \
 \
void vec_T::SetMaxLength(indexType l__n) \
{ \
   indexType l__OldLength = length(); \
   SetLength(l__n); \
   SetLength(l__OldLength); \
} \
 \
void vec_T::FixLength(indexType l__n) \
{ \
...
{  \
  ...
  \
void vec_T::RangeError(indexType l__i) const  \
{  \
...
{  \
   T* l__t;  \
   indexType l__xf = l__x.fixed();  \
   indexType l__yf = l__y.fixed();  \
   if (l__xf != l__yf ||   \
...
void append(vec_T& l__v, const T& l__a)  \
{  \
   indexType l__l = l__v.length(); \
   if (l__l >= l__v.allocated()) {  \
      indexType l__pos = l__v.position(l__a);  \
