...
/** 
 * A flag used by the PCPCD project.
 * If true, the GF2X class uses WordVectorStatic instead of WordVector, which statically allocates the GF2 arrays.
 * NOTE - The attempt to use WordVectorStatic has failed. Do not turn on this flag.
 */
//#define USE_STATIC_WV		
...
/** Arnon's addition - The class is in charge of counting field operations */
#define NTL_OPS_COUNTER
#ifdef NTL_OPS_COUNTER
class OpsCounter {

public:

	static bool count;
	static long long additionsCounter;
	static long long multiplicationsCounter;
	static long long divisionsCounter;

	static void startCount() { count = true; additionsCounter=0; multiplicationsCounter=0; divisionsCounter=0; }
	static void endCount()	 { count = false; }
	static void increaseAdd() { if (count) additionsCounter++; }
	static void increaseMul() { if (count) multiplicationsCounter++; }
	static void increaseDiv() { if (count) divisionsCounter++; }

	static long long getTotalOps() { return additionsCounter+multiplicationsCounter+divisionsCounter; }
};
#endif
