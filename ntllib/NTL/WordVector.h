...
class WordVector {  
public:  
   _ntl_ulong *rep;					///An array of unsigned longs. Each bit is a GF2 element. rep[-1] = array length
   void RangeError(long i) const;   ///Prints an error of sort "index out of range in vector: i (len)"

   WordVector(WordVector& x, INIT_TRANS_TYPE) { rep = x.rep; x.rep = 0; }	//constructs a new object out of x.
     
   WordVector() { rep = 0; }											///Default C'tor
   WordVector(INIT_SIZE_TYPE, long n) { rep = 0; DoSetLength(n); }		///Constructs a vector of n elements.
