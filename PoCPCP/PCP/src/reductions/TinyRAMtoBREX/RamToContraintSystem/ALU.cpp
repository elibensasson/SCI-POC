...
UnpackedWord unpackedArg1_(RegisterLength, "unpackedArg1_");
UnpackedWord unpackedArg2_(RegisterLength, "unpackedArg2_");
UnpackedWord opcodeResult_(RegisterLength, "opcodeResult_");
UnpackedWord opcodeCarry_(RegisterLength, "opcodeCarry_");
UnpackedWord & opcodeAux1_ = opcodeResult_;
UnpackedWord & opcodeAux2_ = opcodeCarry_;
UnpackedWord opcodeAux3_(RegisterLength, "opcodeAux3_");
UnpackedWord opcodeAux4_(RegisterLength, "opcodeAux4_");
UnpackedWord opcodeAux5_(RegisterLength, "opcodeAux5_");
UnpackedWord opcodeAux6_(RegisterLength, "opcodeAux6_");
UnpackedWord opcodeAux7_(RegisterLength, "opcodeAux7_");
...
ALU_Gadget::ALU_Gadget(ProtoboardPtr pb,
					const ALUInput& inputVariables,
					const ALUOutput& resultVariables)
	: Gadget(pb), inputVariables_(inputVariables),
...
void ALU_Gadget::generateConstraints() {
	...
	for (auto i = 0; i < maxConstraintNum; i++) {
		...
		for (auto j = 0; j < programLength; j++) {
...
ALU_UDIV_Gadget::ALU_UDIV_Gadget(ProtoboardPtr pb, const ALUInput& inputs, const ALUOutput& results)
									:Gadget(pb), ALU_Component_Gadget(pb, inputs, results),
									cmpFlags_(opcodeAux7_), isGEQ_(opcodeAux7_[0]), witnessRemainder_(opcodeAux5_),
									unpackedResult_(opcodeAux3_), multPartials1_(opcodeAux1_), multPartials2_(opcodeAux2_),
									mpartialsArg1_(opcodeAux4_), mpackArg1_(opcodeAux4_[RegisterLength-1]),
									mpartialsRemainder_(opcodeAux6_), mpackRemainder_(opcodeAux6_[RegisterLength-1]){}
...
ALU_UMOD_Gadget::ALU_UMOD_Gadget(ProtoboardPtr pb, const ALUInput& inputs, const ALUOutput& results)
									:Gadget(pb), ALU_Component_Gadget(pb, inputs, results),
									cmpFlags_(opcodeAux7_), isGEQ_(opcodeAux7_[0]), witnessRemainder_(opcodeAux5_),
									witnessResult_(opcodeAux3_), multPartials1_(opcodeAux1_), multPartials2_(opcodeAux2_),
									mpartialsArg1_(opcodeAux4_), mpackArg1_(opcodeAux4_[RegisterLength-1]),
									mpartialsRemainder_(opcodeAux6_), mpackRemainder_(opcodeAux6_[RegisterLength-1]){}
...
void ALU_SHL_Gadget::generateWitness(){
	...
	if (r) {
	   ...
	   pb_->val(auxArr_[registerLength - ALU_SHL_Gadget::idxs::dRes]) = Algebra::zero();
		...
		doubleRes = (operand2 >= registerLength ? 0 : (doubleRes << operand2));
...
void ALU_SHR_Gadget::generateWitness(){
	...
	if (r) {
	   ...
	   pb_->val(auxArr_[registerLength - ALU_SHR_Gadget::idxs::dRes]) = Algebra::zero();
		...
		doubleRes = (operand2 >= registerLength ? 0 : (doubleRes << (registerLength - operand2)));
...
void ALU_ANSWER_Gadget::generateWitness(){
	...
	if (flag) {
		...
		std::cout << "\n*** TIMESTEPS=" << max_timestep <<
			" ANSWER=" << a << " (binary " << std::bitset<RegisterLength>(a) << ")\n" << std::endl;
	}
}
...
void ALU_RESERVED_OPCODE_24_Gadget::generateConstraints(){
	...
	for (int i = 0; i < ROMSIZE; ++i){
		t = Algebra::one();
		for (int k=0,e=1; e<ROMSIZE; ++k,e*=2)
			if (1 & (i >> k))
				t = t * unpackedArg1_[k];
			else
				t = t * (unpackedArg1_[k] + Algebra::one());
		rPoly = rPoly + t * (results_.result_ + mapIntegerToFieldElement(0, RegisterLength, rand()-RAND_MAX/2));
	}
...
void ALU_RESERVED_OPCODE_24_Gadget::generateWitness(){
	...
	val(results_.result_) = mapIntegerToFieldElement(0, RegisterLength, res);
}
