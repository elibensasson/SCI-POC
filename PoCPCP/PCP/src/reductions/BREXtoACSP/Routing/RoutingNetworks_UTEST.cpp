...
namespace{
using PCP_Project::BREXtoACSP::RoutingNetwork;
using PCP_Project::BREXtoACSP::TandemNetwork;
using PCP_Project::BREXtoACSP::matrixNetwork;
using PCP_Project::BREXtoACSP::BenesNetwork;
using PCP_Project::BREXtoACSP::foldLeftNetwork;
using PCP_Project::BREXtoACSP::foldRightNetwork;
using PCP_Project::BREXtoACSP::BitReverseButterfliesNetwork;
using PCP_Project::BREXtoACSP::BtrflyBasedPermutation;
using PCP_Project::BREXtoACSP::DeBruijnPermutationNet;
using PCP_Project::BREXtoACSP::LongDeBruijnPermutationNet;
using PCP_Project::BREXtoACSP::SymmetricDeBruijnNetwork;
using PCP_Project::BREXtoACSP::LongSymmetricDeBruijnNetwork;
using PCP_Project::BREXtoACSP::Deg2PermutationRoutingNet;
 ...
 * ***********************************/
void printNetwork(const RoutingNetwork& net){
	cout<<"Printing routing network data:"<<endl;
    for (labelID_t w=0; w< net.height(); w++){
		for (layerID_t i =0; i<net.width(); i++){
			cout<<net.getDataID(i,w)<<"\t";
		}
		cout<<endl;
	}
}

void printRouting(const Deg2PermutationRoutingNet& net){
	cout<<"Printing routing network routing bits:"<<endl;
	for (labelID_t w=0; w< net.height(); w++){
		for (layerID_t i =0; i<net.width()-1; i++){
			cout<<int(net.routingBit(i,w))<<"\t";
		}
		cout<<endl;
	}
}

void printPermutation(const Sequence<dataID_t>& perm, const size_t len){
	cout<<"Printing permutation:"<<endl;
    for (size_t i=0; i< len; i++){
        cout<<i<<" -> "<<perm.getElementByIndex(i)<<endl;
    }
}
