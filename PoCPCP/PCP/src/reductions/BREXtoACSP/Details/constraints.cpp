...
/**
 * Construct the constraints polynomial from a BREX instance.
...
const CS_testLocations& testLocations){
    ...
    for(size_t poly_indx=0; poly_indx<constraints.size(); poly_indx++){
        ...
        // because the deffinition of BREX does not require consistency
...
const vector<FieldElement>& paddingPi){
    ...
    const auto layersPairVanishingPoly = bigPoly.addSubspacePoly(spacesGenerator.getNetworkLayersPairBasis());
     ...
     ******************************************************************/
    {
        const size_t routingBitsDoubleLayerIndex = commonDef.variablesPerm().size();
        const FieldElement routingBitsDoubleLayerOffset = instanceMapping.mapPermutationLayerId_spaceElement(2*routingBitsDoubleLayerIndex);
        ...
        const FieldElement layer0_zeroColShift = instanceMapping.mapPermutationLayerId_spaceElement(2*routingBitsDoubleLayerIndex);
        const FieldElement layer1_zeroColShift = instanceMapping.mapPermutationLayerId_spaceElement(2*routingBitsDoubleLayerIndex+1);
        const FieldElement layer0_lastColShift = lastColShift + instanceMapping.mapPermutationLayerId_spaceElement(2*routingBitsDoubleLayerIndex);
        const FieldElement layer1_lastColShift = lastColShift + instanceMapping.mapPermutationLayerId_spaceElement(2*routingBitsDoubleLayerIndex+1);
        ...
        const ACSPSummandsPolynomial::AuxPolyVec auxPolys = {layer0_zeroCol_poly, layer1_zeroCol_poly, layer0_lastCol_poly, layer1_lastCol_poly};

        //construct the booleanity test polynomial
        const booleanityTestPoly test(neighbors.locationOfId() + 1);

        // add the summand
        bigPoly.addSummand(layersPairVanishingPoly, routingBitsDoubleLayerOffset, test, auxPolys); 
    }
    ...
    for(size_t currVar=0; currVar < commonDef.variablesPerm().size(); currVar++){
        ...
        const equivalenceTestPoly test(1+neighbors.locationOfId(), 1+neighbors.locationOfTwinLayer());
    ...
    for(size_t currVar=0; currVar < 2*commonDef.variablesPerm().size(); currVar++){
        ...
        // get the constraint polynomial
        const constValTestPoly test(1+neighbors.locationOfId(), expectedVal);
    ...
    for(size_t currLayer=0; currLayer < amountOfLayersToCheck; currLayer++){
        ...
        for(size_t cosetId=0; cosetId < DeBruijnShifts.size(); cosetId++){
            ...
            const size_t currVal_var = neighbors.locationOfId() + 1;
