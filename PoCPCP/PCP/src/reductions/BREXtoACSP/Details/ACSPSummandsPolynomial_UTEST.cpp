/** tests for the class ACSPSummandsPolynomial */
...
namespace{
	...
	using PCP_Project::BREXtoACSP::ACSPSummandsPolynomial;
...
#define FIELD_SIZE 64
	...
	//random multi-variate poly of form c1*x_1 +..+ c_t*x_t
	unique_ptr<lightCircPoly> randPoly(int varNum){

		//construct as univariate polynomial
		UnivariatePolynomialGeneral poly;
		poly.setCoefficient(1, generateRandom());
		unique_ptr<lightCircPoly> res(new lightCircPoly(poly));
		unique_ptr<lightCircPoly> res2;
		for (int i = 0; i < varNum; i++){
			//construct and return the circuitPolynomial
			poly.setCoefficient(1, generateRandom());
			res2 = std::unique_ptr<lightCircPoly>(new lightCircPoly(poly));
			res->addDistinct(*res2);
		}
		return res;
	}
...
//******TESTS*******//
	...
	TEST(ACSP, summandPolyNoConstInDenum){
		FiniteField(FIELD_SIZE).setContext();
