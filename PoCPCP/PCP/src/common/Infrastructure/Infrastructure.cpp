...
::std::string FMT(const char* format, ...) {
    ...
    va_end(args);
    _COMMON_ASSERT(strChk >= 0 && strChk < MAX_FMT, "String length larger than buffer. Shorten"
                                        " string or increase buffer size defined in \"MAX_FMT\".");
    return ::std::string(buf);
}
#else // not DEBUG
::std::string FMT(const char* format, ...) {return "";}
#endif
...
long safeConvert(const int64_t num) {
	//ARIEL:2
	_COMMON_ASSERT(num <= INT_MAX && num >= INT_MIN, "integer not in 32-bit range");
...
void ErrorHandling::printStacktrace() {
	...
	throw ("_COMMON FATAL"); //meanwhile at least throwing exception so can get MSVS to break so can see stacktrace
#endif // __GNUC__
}
