...
#ifndef BREXINSTANCE_HPP__
#define BREXINSTANCE_HPP__

namespace PCP_Project{

/**
 * @class BREXInstance
 * @brief class for BREX instance
 *
 * A BREX instance is a tuple \f$(\mathbb{F},d,\mathcal{V},\mathcal{C}_\mathcal{A}, \mathcal{C}_\pi, B, padding_\pi)\f$ such that:
 * 
 ...
 * \f]
 *
 * A BREX instance \f$(\mathbb{F},d,\mathcal{V},\mathcal{C}_\mathcal{A}, \mathcal{C}_\pi, B)\f$
...
public:
	 ...
	 */
	BREXInstance(BREXInstance&& src) : // = default;
		contextField_(std::move(src.contextField_)),
		vectorsLen_(std::move(src.vectorsLen_)),
		domainSizeIndicator_(src.domainSizeIndicator_),
		constraintsAssignment_(std::move(src.constraintsAssignment_)),
		constraintsPermutation_(std::move(src.constraintsPermutation_)),
		boundaryConstraints_(std::move(src.boundaryConstraints_)),
        paddingPi_(std::move(src.paddingPi_)){};
...
private:
	 ...
	 */
	BREXInstance(const BREXInstance& src) _COMMON_CXX11_DELETED;
