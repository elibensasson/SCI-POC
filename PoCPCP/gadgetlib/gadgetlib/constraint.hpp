...
public:
    ...
    **/
    ...
    typedef ::std::set< ::std::unique_ptr<Algebra::PolynomialInterface> > PolyPtrSet;
    ...
    PolyPtrSet getConstraintPolynomials() const {
		...
		for(const Constraint& pConstraint : constraintsPtrs_) {
		    retset.insert(pConstraint.constraint().clone());  
#ifdef printConstraints
			std::cout << pConstraint.constraint().asString() << std::endl;
#endif
        }
        return retset;
    }
