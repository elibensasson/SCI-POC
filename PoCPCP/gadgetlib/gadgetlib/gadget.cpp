...
CompressionPacking_Gadget::CompressionPacking_Gadget(ProtoboardPtr pb,
														...
														Opcode opcode)
	:Gadget(pb), unpacked_(unpacked), packed_(packed), packingMode_(packingMode),opcode_(opcode){}
...
void CompressionPacking_Gadget::generateConstraints(){
	...
	const int bitsPerFieldElem = long(::NTL::GF2E::degree());
...
void DoubleUnpack_Gadget::generateWitness(){
	...
	const size_t bitsPerFieldElem = long(::NTL::GF2E::degree());
...
MultiplicationPacking_Gadget::MultiplicationPacking_Gadget(ProtoboardPtr pb,
	...
	const Opcode opcode)
	: Gadget(pb), unpacked_(unpacked), partials_(partials), packed_(packed), isSigned_(isSigned), packingMode_(packingMode), opcode_(opcode){
...
void MultiplicationPacking_Gadget::generateConstraints(){
	...
	int i; Algebra::FElem g_2_i; Algebra::LinearCombination c;
	...
	for (int i = 0; i < unpacked_.size(); i++)
...
void MultiplicationPacking_Gadget::generateWitness(int n){
	if (PackingMode::UNPACK == packingMode_){
		...
		int numberOfDigits = n == 0 ? 0 : floor(log2(n)) + 1;
...
void DoubleMultPack_Gadget::generateConstraints(){
	int j; Algebra::FElem g_2_i; Algebra::LinearCombination c;
	Algebra::LinearCombination prev = unpacked1_[0] * Algebra::xFE() + (Algebra::one() + unpacked1_[0]);
	for (int i = 1; i < unpacked1_.size(); i++){
		g_2_i = getGenerator2toTheNthPower(i);
...
void DoubleMultPack_Gadget::generateWitness(){
	...
	for (int j = 0; j < unpacked2_.size()-1; j++){
...
Multiplication_Gadget::Multiplication_Gadget(ProtoboardPtr pb,
	...
	const bool isSigned,
	const Opcode opcode)
	: Gadget(pb), unpacked1_(unpacked1), unpacked2_(unpacked2), partials1_(partials1), partials2_(partials2),
	  isSigned_(isSigned), idxLast(unpacked1.size() - 1), opcode_(opcode){
