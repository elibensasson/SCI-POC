...
FFT::FFT(Basis& b, const fft_operation_t operation) :
		...
		c_p_cpy((Element*)malloc(sizeof(Element)*(1UL<<(b.getSize()))))
#ifdef __GPU
...
FFT::~FFT() {
	 ...
	 *
	 */
void FFT::AlgFFT(Polynomial* P, len_t p_len){
		Element* c = *P;
...
#pragma omp parallel
		{
		idx_t idx = omp_get_thread_num();
		idx_t num = omp_get_num_threads();
	...
	void FFT::GPartition_cpu(Element* G, len_t log_g_len, Element* res){
		len_t g_len = 1<<log_g_len;
		for(idx_t i =  0; i < g_len ; i+=2){
			res[i>>1]=G[i];
...
#pragma omp parallel for
	...
	void FFT::GPartition_cpuOMP(Element* G, len_t log_g_len,Element* res){
		len_t g_len = 1<<log_g_len;
...
#pragma omp parallel private(tid,low,high)
	...
	void FFT::WFromUV_cpu_serial(Polynomial P, len_t log_g_len, len_t size){
		Polynomial G;
		Element* t = subspaces[this->basis.getSize()-log_g_len];
		len_t half_g_len = 1UL<<(log_g_len-1);
		for(idx_t j = 0; j<(1UL<<size) ; j+=(1UL<<log_g_len)){
	...
	void FFT::WFromUV_cpu(Polynomial P, len_t log_g_len, len_t size){
		Polynomial G;
		idx_t tid,low,high;
		len_t half_g_len = 1UL<<(log_g_len-1);
		if(1UL<<(size-log_g_len)>= omp_max_threads)
...
#pragma omp parallel private(tid,low,high)
	...
	void FFT::ifft_cpu(Polynomial P){
		...
		Element* it;
		Element* private_it;
		const int min_log_general_level = MIN(log_omp_max_threads,size-1);
		omp_set_num_threads(omp_max_threads);
...
#pragma omp parallel for
			for (long long i = 0; i < bound; i+=step){
				FFT::ifft_serial(P+i,size-min_log_general_level,c_p_cpy+i);
			}
		}
		for(idx_t i = size - min_log_general_level + 1; i<=size ; ++i){
			...
			for (long long j = 0; j < (1UL << (size - i)); ++j, it += (1UL << i))
			{
...
#pragma omp parallel
			{
                ...
                memcpy(&(P[low]),&(c_p_cpy[low]),sizeof(Element)*(high-low));
			}
			if(omp_max_threads > (1UL<<size))
				omp_set_num_threads(omp_max_threads);
		}
	}
	void FFT::fft_serial(Polynomial P,len_t log_size, Element* c_p_cpy){
		const len_t size = log_size;
		const len_t orig_size = this->basis.getSize();
		Element* p_cpy = P;
		Element* it;
	...
	void FFT::fft_cpu(Polynomial P){
		const len_t size = basis.getSize();
		Element* p_cpy = P;
		Element* it = c_p_cpy;
		Element* private_it;
		const int min_log_general_levels = MIN(log_omp_max_threads,size-1);
		omp_set_num_threads(omp_max_threads);
