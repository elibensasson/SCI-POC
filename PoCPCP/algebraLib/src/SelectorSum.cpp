...
namespace Algebra{
	...
	vector<FieldElement> assignmentToVector(const Algebra::VariableAssignment& assignment){
		...
		for (const auto& v : assignment){
			if (v.first.getIndex() > retVal.capacity())
	...
	SelectorSum::SelectorSum(const vector<CircuitPolynomial>& constraints, const vector<Variable>& selectorVariableVector, const vector<long>& selectorToConstraint,
		...
		constraints(constraints), constraintNum(constraints.size()), selectorToConstraint(selectorToConstraint), selectorVarNum(selectorVariableVector.size()),
		selectorNumBound(std::pow(2, selectorVariableVector.size())), selectorNum(selectorToConstraint.size()), selectorRelevant(selectorRelevant){
		...
		for (auto i = 0; i < selectorVariableVector.size(); i++){
	...
	SelectorSum::SelectorSum(const vector<CircuitPolynomial>& constraints, const vector<Variable>& selectorVariableVector, const vector<long>& selectorToConstraint) :
		constraints(constraints), constraintNum(constraints.size()), selectorToConstraint(selectorToConstraint), selectorVarNum(selectorVariableVector.size()),
		selectorNumBound(std::pow(2, selectorVariableVector.size())), selectorNum(selectorToConstraint.size()){
		...
		for (auto i = 0; i < selectorToConstraint.size();i++)
	...
	void SelectorSum::setNewIndices(std::map<VarIndex_t, VarIndex_t>& old2New){
		for (auto i = 0; i < selectorVars.size(); i++)
