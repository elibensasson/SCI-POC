...
//#include "../Algebra/details/AlgebraUtils.hpp"
...
/**
* @class FieldElement
* @brief Describes an element of a field
* As mentioned it describes an element of some
* field, and not of a specific field.
...
* field element can tell what is its context field.
...
public:
	...
	*/
	...
	/**
	* @brief   conversion construction from NTL element
	*/
	explicit FieldElement(const ::NTL::GF2E& e) : element_(NTLtoFFF(e)){};
	...
	*/
	...
	/**
	* @brief   conversion to ::NTL::GF2E
	*/
	explicit operator const ::NTL::GF2E()const{ return FFFtoNTL(element_); }
...
private:
    ...
    static FFF::Element NTLtoFFF(const NTL::GF2E& ntlE){
        NTL::GF2X coeffs = NTL::GF2E(ntlE).LoopHole();
        FFF::Element result;
		result.c[0] = 0;
        for(size_t i=0; i< numBits; i++){
            result.c[0] <<= 1;
            result.c[0] += rep(NTL::coeff(coeffs,numBits-i-1));
        }

        return result;
    }

    static NTL::GF2E FFFtoNTL(const FFF::Element& fffE){
        NTL::GF2X result(0,0);
        size_t numToWrite = fffE.c[0];
        for(size_t i=0; (i< numBits) && (numToWrite!=0); i++){
            if( numToWrite%2 == 1){
                NTL::SetCoeff(result,i);
            } 
            numToWrite>>=1;
        }
        return NTL::to_GF2E(result);
    }
