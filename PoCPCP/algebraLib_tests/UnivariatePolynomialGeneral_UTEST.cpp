...
TEST(Algebra,UniPolyConstructionByRoots_Soundness){
	...
	//Define polynomial
	UnivariatePolynomialGeneral poly(roots);	//polynomial for test
	EXPECT_EQ(roots.size(), PolynomialDegree::integral_t(poly.getDegree()));
	for(int i=0 ;i<amountOfEvals; i++){
...
TEST(Algebra,UniPolyInterpulation){
    //constants
    const int fieldExtensionDegree = 64;
    const int basisMaxSize = 10;
    
    //define the context field
    FiniteField contextField(fieldExtensionDegree);
    ...
    elementsSet_t basis;
    UnivariatePolynomialGeneral::evaluation_t mapping(compareFieldElements);
    ...
    //verify degree
    EXPECT_TRUE(PolynomialDegree::integral_t(poly.getDegree()) <= mapping.size());
